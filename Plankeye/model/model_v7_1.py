...
"""PlankEye V7.1 - corner-focused industrial quadrilateral detector.
...
MODEL_VERSION = "v7.1-resnet50-fpn-pan-dual-corners-cornerfocus"
...
# V7.1: corner localization is intentionally the dominant objective.
LOSS_W_CORNER_DELTA = 10.00

LOSS_W_CORNER_ABS = 4.00

LOSS_W_RECONSTRUCTION = 8.00

# Extra corner-focused penalties. These are computed on the final fused quad.
# - WORST_CORNER prevents one badly misplaced corner from being hidden by the mean.
# - RELATIVE_CORNER gives small objects more importance by normalizing by object diagonal.
# - EDGE_MIDPOINT stabilizes the four sides without forcing rectangles.
LOSS_W_WORST_CORNER = 3.00

LOSS_W_RELATIVE_CORNER = 1.50

LOSS_W_EDGE_MIDPOINT = 1.00

LOSS_W_SIZE = 0.50

LOSS_W_GEOMETRY = 2.50

LOSS_W_DUAL_CONSISTENCY = 1.00

LOSS_W_CENTER_CONSISTENCY = 0.50

LOSS_W_QUALITY = 0.25

# Geometry sub-loss weights. The target angles are learned as labelled;
# V7.1 does NOT force 90-degree corners, so perspective remains valid.
GEOM_W_AREA = 1.00

GEOM_W_DIRECTION = 0.75

GEOM_W_LENGTH = 0.75

GEOM_W_DIAGONAL = 0.50

GEOM_W_ANGLE = 1.00

GEOM_W_CONVEXITY = 0.50

# Stable normalizer for object-relative corner errors.
RELATIVE_CORNER_MIN_DIAGONAL = 0.02
...
def combined_loss_v7_1(
    ...
    # -------------------------------------------------------------------------
    # V7.1 CORNER-FOCUSED LOSSES
    # -------------------------------------------------------------------------
    # The older V7 mainly minimized the average coordinate error. That can look
    # acceptable while a single corner remains far from the true endpoint.
    # V7.1 explicitly penalizes the worst corner of every object.
    corner_vectors = fused_pred - best_abs_target
    corner_distances = torch.linalg.vector_norm(corner_vectors, dim=-1)

    worst_corner_error = corner_distances.max(dim=1).values
    loss_worst_corner = F.smooth_l1_loss(
        worst_corner_error,
        torch.zeros_like(worst_corner_error),
        beta=0.02,
    )

    # Object-relative corner error: a 10-pixel miss is proportionally much more
    # damaging on a small plank than on a very large one. Normalizing by the GT
    # bounding-box diagonal makes small/medium objects matter during training.
    target_min = best_abs_target.amin(dim=1)
    target_max = best_abs_target.amax(dim=1)
    target_diag = torch.linalg.vector_norm(target_max - target_min, dim=-1)
    target_diag = target_diag.detach().clamp(min=RELATIVE_CORNER_MIN_DIAGONAL)
    relative_corner_error = corner_distances / target_diag.unsqueeze(1)
    loss_relative_corner = F.smooth_l1_loss(
        relative_corner_error,
        torch.zeros_like(relative_corner_error),
        beta=0.05,
    )

    # Edge midpoint supervision reinforces the four side locations while still
    # allowing arbitrary perspective quadrilaterals.
    pred_midpoints = 0.5 * (fused_pred + torch.roll(fused_pred, shifts=-1, dims=1))
    target_midpoints = 0.5 * (
        best_abs_target + torch.roll(best_abs_target, shifts=-1, dims=1)
    )
    loss_edge_midpoint = F.smooth_l1_loss(
        pred_midpoints,
        target_midpoints,
        beta=0.02,
    )

    # A dedicated validation quantity is logged so train_v7_1.py can save a
    # best_corners.pt independently from the generic best total-loss checkpoint.
    corner_focus_loss = (
        LOSS_W_CORNER_DELTA * loss_corner_delta
        + LOSS_W_CORNER_ABS * loss_corner_abs
        + LOSS_W_RECONSTRUCTION * loss_reconstruction
        + LOSS_W_WORST_CORNER * loss_worst_corner
        + LOSS_W_RELATIVE_CORNER * loss_relative_corner
        + LOSS_W_EDGE_MIDPOINT * loss_edge_midpoint
    ...
    logs = {
        ...
        "worst_corner": float(loss_worst_corner.detach().item()),

        "relative_corner": float(loss_relative_corner.detach().item()),

        "edge_midpoint": float(loss_edge_midpoint.detach().item()),

        "corner_focus": float(corner_focus_loss.detach().item()),
...
# Backward-compatible aliases. The architecture stays checkpoint-compatible
# with V7; only the training objective is more corner-focused.
combined_loss_v7 = combined_loss_v7_1


def combined_loss_v5(preds, batch_objects, device):

    return combined_loss_v7_1(preds, batch_objects, device)
...
def model_metadata() -> dict:
    ...
    return {
        ...
        "training_focus": "corner precision / worst-corner / object-relative geometry",
        ...
        "loss_weights": {
            ...
            "worst_corner": LOSS_W_WORST_CORNER,

            "relative_corner": LOSS_W_RELATIVE_CORNER,

            "edge_midpoint": LOSS_W_EDGE_MIDPOINT,
