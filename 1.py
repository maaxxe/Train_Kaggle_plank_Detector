...
def build_optimizer(model):
    base = unwrap_model(model)
    backbone_params = list(base.backbone.parameters())
    backbone_ids = {id(p) for p in backbone_params}
    head_params = [p for p in base.parameters() if id(p) not in backbone_ids]

    param_groups = [
        {"params": head_params, "lr": LR_HEAD, "group_name": "head"},
        {"params": backbone_params, "lr": LR_BACKBONE, "group_name": "backbone", "lr_mult": 1.0}
    ]
...
def load_ckpt(model, optimizer, scaler, ema, path: Path):
    ...
    return int(checkpoint.get("epoch", 0)) + 1, checkpoint.get("histories", empty_histories), float(checkpoint.get("best_quality", float("-inf")))
...
def main():
    ...
    try:
        ...
        start_epoch, histories, best_quality = load_ckpt(raw_model, optimizer, scaler, ema, LAST_CHECKPOINT_PATH)
